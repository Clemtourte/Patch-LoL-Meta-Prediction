...
class UserData:
    ...
    def get_matches_by_date(self, queue_id, start_time, end_time):
        matches = []
        start_index = 0
        logging.info(f"=== Starting match search ===")
        logging.info(f"PUUID: {self.puuid}")
        ...
        while True:
            ...
            if not batch:
                logging.warning("No matches found in batch")
                break
...
def save_match_data(user_data, match_ids, session):
    ...
    for i, match_id in enumerate(match_ids, 1):
        ...
        if existing_match:
            logging.info(f"Match {general_info['game_id']} already exists. Skipping.")
            continue
        else:
